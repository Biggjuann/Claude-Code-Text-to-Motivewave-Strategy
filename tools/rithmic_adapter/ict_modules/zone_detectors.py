...
Copied from nautilus_backtest/zone_detectors.py — no Nautilus dependencies.
Import paths adjusted for ict_modules package.
"""

import math
from ict_modules.zones import Zone, ZoneType
...
def check_ob_to_breaker(ob_zones: list[Zone], close: float, bar_index: int) -> list[Zone]:
    ...
    for ob in ob_zones:
        ...
        if ob.is_bullish and close < ob.bottom:
            ob.violated = True
            ob.is_valid = False
            new_breakers.append(Zone(ob.top, ob.bottom, bar_index, False, ZoneType.BREAKER))
        elif not ob.is_bullish and close > ob.top:
            ob.violated = True
            ob.is_valid = False
            new_breakers.append(Zone(ob.top, ob.bottom, bar_index, True, ZoneType.BREAKER))
    return new_breakers
...
def detect_structure_breakers(
    ...
    last_swing_high_bar: int, last_swing_low_bar: int,
    require_displacement: bool = True, lookback: int = 20,
) -> list[Zone]:
    new_breakers = []
    ...
    if swept_low and cur_close > last_swing_high:
        ...
        if body_size > 5 or not require_displacement:
            ...
            new_breakers.append(Zone(breaker_top, sweep_low_price, bar_index, True, ZoneType.BREAKER))
    ...
    if swept_high and cur_close < last_swing_low:
        ...
        if body_size > 5 or not require_displacement:
            ...
            new_breakers.append(Zone(sweep_high_price, breaker_bottom, bar_index, False, ZoneType.BREAKER))

    return new_breakers


def detect_fvgs(highs: list[float], lows: list[float], bar_index: int,
                min_gap: float = 2.0) -> list[Zone]:
    new_zones = []
    n = len(highs)
    if n < 3:
        return new_zones
        ...
        for fvg in fvg_zones:
            ...
            if overlap_top > overlap_bottom:
                ...
                if key not in existing_levels:
                    ...
                    new_bprs.append(Zone(overlap_top, overlap_bottom, bar_index, ifvg.is_bullish, ZoneType.BPR))
    return new_bprs
...
def update_swing_points(
    highs: list[float], lows: list[float], bar_index: int,
    left: int = 2, right: int = 2,
    last_swing_high: float = math.nan, last_swing_low: float = math.nan,
    last_swing_high_bar: int = -1, last_swing_low_bar: int = -1,
    prev_swing_high: float = math.nan, prev_swing_low: float = math.nan,
):
    n = len(highs)
    if n < left + right + 1:
        return None
