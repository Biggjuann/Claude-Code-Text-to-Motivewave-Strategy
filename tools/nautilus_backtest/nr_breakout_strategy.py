"""
Narrow Range Breakout (NR4) Strategy -- NautilusTrader port.

Tracks bar ranges over the last N bars. When the current bar has the
narrowest range in N bars (compression / contraction), it sets pending
breakout levels above and below that bar. On the next bar, enter long
if close > NR high, short if close < NR low. This exploits the
volatility cycle: compression leads to expansion.

Stop at opposite end of the NR bar. TP at R:R ratio.

Reference: Toby Crabel / Tony Crabel -- "Day Trading with Short Term
Price Patterns and Opening Range Breakout"
"""
...
class NRBreakoutConfig(StrategyConfig, frozen=True):
    instrument_id: InstrumentId
    bar_type: BarType

    # NR lookback
    lookback: int = 4              # NR4 = narrowest of last 4 bars
    target_rr: float = 2.0
...
class NRBreakoutStrategy(Strategy):

    def __init__(self, config: NRBreakoutConfig):
        ...
        # Recent RTH bar data (for NR detection)
        self.recent_highs: list[float] = []
        self.recent_lows: list[float] = []
        self.recent_ranges: list[float] = []

        # Pending breakout levels
        self.nr_high: float = math.nan
        self.nr_low: float = math.nan
        self.breakout_pending: bool = False
    ...
    def on_start(self):
        ...
        self.log.info("Narrow Range Breakout (NR4) started")
    ...
    def on_bar(self, bar: Bar):
        ...
        if 935 <= bar_time_int <= 1600:
            ...
            self.recent_ranges.append(bar_range)

            max_hist = cfg.lookback + 2
            if len(self.recent_ranges) > max_hist:
                self.recent_highs.pop(0)
                self.recent_lows.pop(0)
                self.recent_ranges.pop(0)

            # Check for NR pattern: current bar is narrowest of last N
            if len(self.recent_ranges) >= cfg.lookback:
                window = self.recent_ranges[-cfg.lookback:]
                current_range = window[-1]
                if current_range <= min(window):
                    # Set pending breakout
                    self.nr_high = h
                    self.nr_low = l
                    self.breakout_pending = True
                    self.log.info(
                        f"NR{cfg.lookback} detected: H={h:.2f}, L={l:.2f}, "
        ...
        # ===== Entry: breakout of NR bar =====
        if not self.breakout_pending:
            return
        if math.isnan(self.nr_high) or math.isnan(self.nr_low):
            return
        ...
        nr_range = self.nr_high - self.nr_low
        if nr_range <= 0:
            return

        # Long: close breaks above NR high
        if c > self.nr_high:
            stop = self.nr_low  # opposite end of NR bar
            risk = abs(c - stop)
            if risk > 0:
        ...
        # Short: close breaks below NR low
        elif c < self.nr_low:
            stop = self.nr_high  # opposite end of NR bar
            risk = abs(stop - c)
            if risk > 0:
                tp = c - cfg.target_rr * risk
    ...
    def _enter_trade(self, close: float, side: OrderSide, direction: int,
        ...
        self.log.info(
            ...
            f"risk={risk:.2f}, NR_H={self.nr_high:.2f}, NR_L={self.nr_low:.2f}"
        )
