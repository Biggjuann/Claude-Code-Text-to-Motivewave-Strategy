"""
EMA Bounce (Trend Continuation) Strategy -- NautilusTrader port.

Computes an EMA on 5-minute RTH bars. Trend is bullish when close > EMA,
bearish when close < EMA. Entry occurs when price pulls back to within a
threshold of the EMA and the bar closes back in the trend direction:
  - Long: low touches near EMA from above, bar closes green above EMA.
  - Short: high touches near EMA from below, bar closes red below EMA.
Stop beyond the pullback extreme, TP at R:R ratio.
"""
...
class EMABounceConfig(StrategyConfig, frozen=True):
    ...
    ema_period: int = 21
    atr_period: int = 14
    touch_atr_mult: float = 0.5     # how close to EMA counts as "touch"
    target_rr: float = 3.0
...
class EMABounceStrategy(Strategy):
    ...
    def __init__(self, config: EMABounceConfig):
        super().__init__(config)

        self.instrument_id = config.instrument_id
        self.bar_type = config.bar_type

        # Daily OHLC from RTH bars (for daily ATR)
        ...
        # EMA on 5-min RTH bars
        self.ema_value: float = math.nan
        self.ema_mult: float = 2.0 / (config.ema_period + 1)
        self.rth_bar_count: int = 0

        # Intraday ATR (5-min bars) for touch threshold
    ...
    def on_start(self):
        ...
        self.log.info("EMA Bounce (Trend Continuation) started")
    ...
    def on_bar(self, bar: Bar):
        ...
        # ===== Update EMA and intraday ATR on RTH bars =====
        if 935 <= bar_time_int <= 1600:
            ...
            if math.isnan(self.ema_value):
                ...
                self.ema_value = c * self.ema_mult + self.ema_value * (1.0 - self.ema_mult)
        ...
        # ===== Entry conditions =====
        if math.isnan(self.ema_value) or math.isnan(self.intraday_atr):
            return
        if math.isnan(self.daily_atr):
            return
        if self.rth_bar_count < cfg.ema_period:
            return
        ...
        touch_dist = cfg.touch_atr_mult * self.intraday_atr
        if touch_dist <= 0:
            return

        ema = self.ema_value

        # LONG: bullish trend, pullback touches near EMA from above, closes green above EMA
        if c > ema and c > o:  # close above EMA and green bar
            if l <= ema + touch_dist:  # low touched near EMA
                stop = l  # stop below pullback low
                risk = abs(c - stop)
                if risk > 0:
                    tp = c + cfg.target_rr * risk
                    self._enter_trade(c, OrderSide.BUY, 1, stop, tp)

        # SHORT: bearish trend, pullback touches near EMA from below, closes red below EMA
        elif c < ema and c < o:  # close below EMA and red bar
            if h >= ema - touch_dist:  # high touched near EMA
                stop = h  # stop above pullback high
                risk = abs(stop - c)
                if risk > 0:
                    tp = c - cfg.target_rr * risk
    ...
    def _enter_trade(self, close: float, side: OrderSide, direction: int,
        ...
        self.log.info(
            ...
            f"risk={risk:.2f}, EMA={self.ema_value:.2f}, "
