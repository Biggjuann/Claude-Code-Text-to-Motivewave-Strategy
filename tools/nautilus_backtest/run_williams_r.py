"""
Williams %R Strategy — NautilusTrader Backtest Runner.

Usage:
    python run_williams_r.py [--start 2024-01-01] [--end 2026-01-01]
    python run_williams_r.py --start 2024-01-01 --end 2026-01-01 --mes --dollars-per-contract 5000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Williams %R Strategy Backtest")
    parser.add_argument("--start", default=DEFAULT_START, help="Start date YYYY-MM-DD")
    parser.add_argument("--end", default=DEFAULT_END, help="End date YYYY-MM-DD")

    # Williams volatility breakout params
    parser.add_argument("--entry-mult", type=float, default=0.50, help="Entry = open ± range * mult (0.25-0.70)")
    parser.add_argument("--stop-mult", type=float, default=0.50, help="Stop = entry ∓ range * stop_mult")
    parser.add_argument("--target-rr", type=float, default=3.0, help="Target R:R multiple (Williams used 3-4)")
    ...
    parser.add_argument("--label", default="williams_r", help="Output subdirectory label")
    ...
    # 5. Create strategy
    strategy_config = WilliamsRConfig(
        instrument_id=es.id,
        bar_type=bar_type,
    ...
    strategy = WilliamsRStrategy(config=strategy_config)
    ...
    print(f"Entry: Open ± {args.entry_mult}x yesterday range")
    print(f"Stop: {args.stop_mult}x range | Target: {args.target_rr}:1 R:R")
    print()
    ...
    if positions_report is not None and not positions_report.empty:
        ...
        title = f"Williams %R — {args.start} to {args.end} ({instrument_label}, {args.bar_minutes}min)"
