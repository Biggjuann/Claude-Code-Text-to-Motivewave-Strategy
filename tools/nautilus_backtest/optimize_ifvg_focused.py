"""
IFVG Retest Strategy — Focused Optimizer (2010-2026).

Reduced grid (16 combos) to fit within reasonable time.
IFVG is slow (~11 min/combo on 16yr data due to zone tracking).
"""
...
# Focused grid — reduced from 108 to 16 combos
PARAM_GRID = {
    "shadow_threshold_pct": [20.0, 30.0],
    "max_wait_bars": [20, 30],
    "tp1_points": [20.0, 25.0],
    "trail_points": [10.0, 15.0],
}

combos = []
for st, mw, tp, tr in itertools.product(
    PARAM_GRID["shadow_threshold_pct"],
    PARAM_GRID["max_wait_bars"],
    PARAM_GRID["tp1_points"],
    PARAM_GRID["trail_points"],
):
    combos.append({"shadow_threshold_pct": st, "max_wait_bars": mw, "tp1_points": tp, "trail_points": tr})

print(f"Total parameter combinations: {len(combos)}")
print(f"Period: {START} to {END} (16 years)")
print(f"Estimated time: {len(combos) * 11:.0f} minutes (~{len(combos) * 11 / 60:.1f} hours)")
...
for i, params in enumerate(combos):
    ...
    strategy_config = IFVGRetestConfig(
        instrument_id=es.id,
        bar_type=bar_type,
        shadow_threshold_pct=params["shadow_threshold_pct"],
    ...
    strategy = IFVGRetestStrategy(config=strategy_config)
    ...
    print(f"[{i+1}/{len(combos)}] st={params['shadow_threshold_pct']:.0f}% "
          f"mw={params['max_wait_bars']} tp={params['tp1_points']:.0f} "
...
# Save full results
output_dir = Path(__file__).parent / "results" / "ifvg_full_sweep"
output_dir.mkdir(parents=True, exist_ok=True)
output_path = output_dir / "ifvg_focused_sweep.csv"
...
print(f"{'='*130}")
print(f"ALL CONFIGURATIONS — IFVG RETEST FOCUSED 2010-2026 (by Sharpe)")
print(f"{'='*130}")
print(f"{'Shad%':>5} {'MWait':>5} {'TP1':>5} {'Trail':>5} {'Trades':>7} {'PnL':>14} {'$/Trade':>9} "
      f"{'$/Year':>10} {'WR%':>6} {'PF':>6} {'Sharpe':>7} {'MaxDD':>12} {'Calmar':>7}")
print("-" * 130)

for _, r in df_sorted.iterrows():
    print(f"{r['shadow_threshold_pct']:>5.0f} {r['max_wait_bars']:>5.0f} {r['tp1_points']:>5.0f} "
