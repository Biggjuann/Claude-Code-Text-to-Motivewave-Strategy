"""
Higher High / Lower Low Breakout Strategy -- NautilusTrader port.

Tracks the highest high and lowest low over the last N RTH bars.
Enters long when close makes a new N-bar high, short when close
makes a new N-bar low. This is a classic channel breakout approach
on an intraday timeframe.

Stop: ATR-based. Target: R:R ratio.

Reference: Classic Donchian channel breakout adapted to intraday bars.
"""
...
class HHBreakoutConfig(StrategyConfig, frozen=True):
    instrument_id: InstrumentId
    bar_type: BarType

    # Channel params
    lookback: int = 20             # N-bar high/low lookback
    atr_period: int = 14
...
class HHBreakoutStrategy(Strategy):

    def __init__(self, config: HHBreakoutConfig):
    ...
    def on_start(self):
        ...
        self.log.info("Higher High / Lower Low Breakout started")
    ...
    def on_bar(self, bar: Bar):
        ...
        # ===== Update bar history and intraday ATR on RTH bars =====
        if 935 <= bar_time_int <= 1600:
            ...
            atr_hist = cfg.atr_period + 2
            if len(self.close_history) > atr_hist:
        ...
        # ===== Entry conditions =====
        if len(self.recent_highs) < cfg.lookback:
            return
        ...
        # N-bar highest high (excluding current bar)
        prev_highs = self.recent_highs[-(cfg.lookback + 1):-1]
        prev_lows = self.recent_lows[-(cfg.lookback + 1):-1]

        if len(prev_highs) < cfg.lookback:
            return

        n_bar_high = max(prev_highs)
        n_bar_low = min(prev_lows)

        # Long: close makes new N-bar high
        if c > n_bar_high:
            stop = c - stop_dist
            risk = abs(c - stop)
        ...
        # Short: close makes new N-bar low
        elif c < n_bar_low:
            stop = c + stop_dist
            risk = abs(stop - c)
            tp = c - cfg.target_rr * risk
    ...
    def _enter_trade(self, close: float, side: OrderSide, direction: int,
        ...
        self.log.info(
            ...
            f"risk={risk:.2f}, iATR={self.intraday_atr:.2f}"
        )
