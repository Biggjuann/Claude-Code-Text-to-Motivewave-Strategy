"""
Opening Range Breakout Strategy — NautilusTrader Backtest Runner.

Usage:
    python run_donchian.py [--start 2024-01-01] [--end 2026-01-01]
    python run_donchian.py --start 2024-01-01 --end 2026-01-01 --mes --dollars-per-contract 5000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Opening Range Breakout Backtest")
    parser.add_argument("--start", default=DEFAULT_START, help="Start date YYYY-MM-DD")
    parser.add_argument("--end", default=DEFAULT_END, help="End date YYYY-MM-DD")

    # Opening range params
    parser.add_argument("--or-end-time", type=int, default=945, help="OR end time (bar close, e.g. 945 = 9:30-9:45)")
    parser.add_argument("--atr-period", type=int, default=14, help="Daily ATR period")
    parser.add_argument("--atr-stop-mult", type=float, default=0.50, help="Stop = Nx daily ATR")
    parser.add_argument("--atr-tp-mult", type=float, default=0, help="TP = Nx daily ATR (0=EOD only)")

    # Trend filter
    parser.add_argument("--no-trend-filter", action="store_true", help="Disable trend filter")
    parser.add_argument("--trend-lookback", type=int, default=5, help="Trend lookback days")

    # Session
    parser.add_argument("--max-trades", type=int, default=1)
    parser.add_argument("--entry-end", type=int, default=1300, help="No entries after this time")
    ...
    parser.add_argument("--label", default="donchian", help="Output subdirectory label")
    ...
    # 5. Create strategy
    strategy_config = DonchianConfig(
        instrument_id=es.id,
        bar_type=bar_type,
    ...
    strategy = DonchianStrategy(config=strategy_config)
    ...
    # 7. Report results
    print("\n" + "=" * 60)
    print("OPENING RANGE BREAKOUT — BACKTEST RESULTS")
    print("=" * 60)
    ...
    print(f"Opening Range: 9:30-{args.or_end_time // 100}:{args.or_end_time % 100:02d}")
    tp_label = f"{args.atr_tp_mult}x ATR" if args.atr_tp_mult > 0 else "EOD"
    print(f"ATR Stop: {args.atr_stop_mult}x daily ATR | TP: {tp_label} | Exit: EOD")
    print(f"Trend Filter: {'ON' if not args.no_trend_filter else 'OFF'} ({args.trend_lookback}-day)")
    print()
    ...
    if positions_report is not None and not positions_report.empty:
        ...
        title = f"Opening Range Breakout — {args.start} to {args.end} ({instrument_label}, {args.bar_minutes}min)"
