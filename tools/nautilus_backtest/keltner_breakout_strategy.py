"""
Keltner Channel Breakout Strategy — NautilusTrader.

Compute EMA(period) and ATR(period). Upper = EMA + mult*ATR, Lower = EMA - mult*ATR.
Enter on close beyond the channel. Stop at EMA (midline), R:R target.
"""
...
class KeltnerStrategy(Strategy):
    ...
    def on_bar(self, bar: Bar):
        ...
        if 935 <= bar_time_int <= 1600:
            ...
            max_buf = max(cfg.ema_period, cfg.atr_period) + 20
            if len(self.closes) > max_buf:
                self.closes = self.closes[-max_buf:]
                self.tr_values = self.tr_values[-max_buf:]

            # EMA
            if len(self.closes) >= cfg.ema_period:
                if not self.ema_initialized:
                    self.ema = sum(self.closes[-cfg.ema_period:]) / cfg.ema_period
        ...
        # Entry conditions
        if not self.ema_initialized or self.atr <= 0:
            return
        ...
        upper = self.ema + cfg.kelt_mult * self.atr
        lower = self.ema - cfg.kelt_mult * self.atr
