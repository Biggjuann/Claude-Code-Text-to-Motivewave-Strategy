"""
Pivot Point Mean Reversion Strategy — NautilusTrader Backtest Runner.

Usage:
    python run_atr_breakout.py [--start 2024-01-01] [--end 2026-01-01]
    python run_atr_breakout.py --start 2024-01-01 --end 2026-01-01 --mes --dollars-per-contract 5000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Pivot Point Mean Reversion Backtest")
    ...
    parser.add_argument("--atr-stop-mult", type=float, default=1.5, help="Stop = Nx daily ATR")

    # Trend filter
    parser.add_argument("--no-trend-filter", action="store_true", help="Disable trend filter")
    parser.add_argument("--trend-filter-days", type=int, default=5, help="Trend filter lookback days")
    ...
    parser.add_argument("--label", default="atr_breakout", help="Output subdirectory label")
    ...
    # 5. Create strategy
    strategy_config = ATRBreakoutConfig(
        instrument_id=es.id,
        bar_type=bar_type,
    ...
    strategy = ATRBreakoutStrategy(config=strategy_config)
    ...
    # 7. Report results
    print("\n" + "=" * 60)
    print("PIVOT POINT MEAN REVERSION — BACKTEST RESULTS")
    print("=" * 60)
    ...
    print(f"Entry: Fade at R1 (short) / S1 (long) → target PP")
    print(f"ATR Stop: {args.atr_stop_mult}x daily ATR | TP: PP | Exit: EOD")
    print(f"Trend Filter: {'ON' if not args.no_trend_filter else 'OFF'} ({args.trend_filter_days}-day)")
    print()
    ...
    if positions_report is not None and not positions_report.empty:
        ...
        title = f"Pivot MR — {args.start} to {args.end} ({instrument_label}, {args.bar_minutes}min)"
