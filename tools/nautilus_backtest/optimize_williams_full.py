"""
Williams Volatility Breakout — Full-Period Optimizer (2010-2026).

Sweeps entry_mult, stop_mult, target_rr, max_trades across 16 years
...
# Parameter grid
PARAM_GRID = {
    "entry_mult": [0.30, 0.40, 0.50, 0.60],
    "stop_mult": [0.30, 0.50, 0.70],
    "target_rr": [2.0, 3.0, 4.0, 5.0],
    "max_trades": [1, 2],
}

combos = []
for em, sm, rr, mt in itertools.product(
    PARAM_GRID["entry_mult"],
    PARAM_GRID["stop_mult"],
    PARAM_GRID["target_rr"],
    PARAM_GRID["max_trades"],
):
    combos.append({"entry_mult": em, "stop_mult": sm, "target_rr": rr, "max_trades": mt})
...
for i, params in enumerate(combos):
    ...
    sys.stdout.write(f"\r[{i+1}/{len(combos)}] em={params['entry_mult']:.2f} "
                     f"sm={params['stop_mult']:.2f} rr={params['target_rr']:.0f} "
...
# Save full results
output_dir = Path(__file__).parent / "results" / "williams_full_sweep"
output_dir.mkdir(parents=True, exist_ok=True)
output_path = output_dir / "williams_full_sweep.csv"
...
print(f"{'='*130}")
print(f"TOP 20 CONFIGURATIONS — WILLIAMS VB FULL PERIOD 2010-2026 (by Sharpe, min 200 trades, PF > 1.0)")
print(f"{'='*130}")
print(f"{'EM':>5} {'SM':>5} {'RR':>4} {'MT':>3} {'Trades':>7} {'PnL':>14} {'$/Trade':>9} "
      f"{'$/Year':>10} {'WR%':>6} {'PF':>6} {'Sharpe':>7} {'MaxDD':>12} {'Calmar':>7}")
print("-" * 130)

for _, r in viable.head(20).iterrows():
    print(f"{r['entry_mult']:>5.2f} {r['stop_mult']:>5.2f} {r['target_rr']:>4.0f} "
...
# Also show by PF
print(f"\n{'='*130}")
print(f"TOP 10 BY PROFIT FACTOR — WILLIAMS VB FULL PERIOD 2010-2026")
print(f"{'='*130}")
