"""
Trend Acceleration Strategy -- NautilusTrader port.
...
This filters for high-conviction trend continuation moves where price
is already trending AND accelerating.
...
class TrendAccelConfig(StrategyConfig, frozen=True):
    instrument_id: InstrumentId
    bar_type: BarType

    # EMA / ATR / acceleration params
    ema_period: int = 21
    atr_period: int = 14
    accel_mult: float = 1.5        # bar range must exceed this * ATR
    target_rr: float = 3.0
...
class TrendAccelStrategy(Strategy):

    def __init__(self, config: TrendAccelConfig):
    ...
    def on_start(self):
        ...
        self.log.info("Trend Acceleration started")
    ...
    def on_bar(self, bar: Bar):
        ...
        bar_range = h - l
        threshold = cfg.accel_mult * self.intraday_atr
        ...
        if c > o and c > ema:
            stop = l  # stop at bar low (opposite end)
            risk = abs(c - stop)
            if risk > 0:
        ...
        # Short: acceleration bar + bearish close + below EMA
            ...
            stop = h  # stop at bar high (opposite end)
            risk = abs(stop - c)
            if risk > 0:
                tp = c - cfg.target_rr * risk
