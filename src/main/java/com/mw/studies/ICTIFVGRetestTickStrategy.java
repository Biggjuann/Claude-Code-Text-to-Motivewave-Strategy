...
/**
 * ICT IFVG Retest Strategy (Tick Entry)
 *
 * Same IFVG zone detection as the bar-close version, but entries and trade
 * management execute on every tick via onBarUpdate(). This allows the strategy
 * to enter the moment price touches a zone rather than waiting for bar close.
 *
 * Zone detection still runs on completed bars (needs 3-bar pattern).
 * Entry + stop/TP/trail all evaluated on each price update.
 *
 * @version 1.0.0
...
@StudyHeader(
    ...
    id = "ICT_IFVG_RETEST_TICK",
    rb = "com.mw.studies.nls.strings",
    name = "ICT_IFVG_RETEST_TICK",
    label = "LBL_ICT_IFVG_RETEST_TICK",
    desc = "DESC_ICT_IFVG_RETEST_TICK",
    menu = "MW Generated",
    overlay = true,
    studyOverlay = true,
    strategy = true,
...
public class ICTIFVGRetestTickStrategy extends Study
{
    // ==================== Input Keys ====================
    ...
    // Tick-entry guard: prevent multiple entries on same bar
    ...
    // ==================== CALCULATE VALUES ====================
    // Detects IFVG zones and tracks their state. Draws zones and historical markers.
    // Does NOT fire signals — entries happen in onBarUpdate() on tick.
    @Override
    protected void calculateValues(DataContext ctx)
    {
        ...
        for (int i = 2; i < size; i++) {
            ...
            // ===== Process active zones: violation, expiration, retest (for historical display) =====
            for (IFVGZone zone : zones) {
                if (zone.state != ZONE_ACTIVE) continue;
                if (i <= zone.barIndex) continue;
                ...
                // Retest check (historical markers only — live entries handled by onBarUpdate)
                if (zone.isBullish && enableLong) {
                    if (low0 <= zone.top && close0 > zone.bottom) {
                        zone.state = ZONE_TRADED;
                        zone.traded = true;
                        boolean canMark = !pastEOD && inSession && localTradesToday < maxTrades;
                        if (canMark) {
                            localTradesToday++;
    ...
    {
        debug("ICT IFVG Retest (Tick Entry) Strategy activated");
    }
    ...
    // ==================== SIGNAL HANDLER (unused — entries via onBarUpdate) ====================
    @Override
    public void onSignal(OrderContext ctx, Object signal) { }

    // ==================== ON BAR UPDATE (Tick-level ENTRY ONLY) ====================
    @Override
    public void onBarUpdate(OrderContext ctx)
    {
        ...
        // If position closed but state not reset
        if (position == 0 && entryPrice > 0) {
            resetTradeState();
        }

        // Only entry logic here — trade management is in onBarClose(OrderContext)
        ...
        // Current tick price
        double price = instr.getLastPrice();
        if (price <= 0) return;

        // Check active zones for retest
        for (IFVGZone zone : zones) {
            if (zone.state != ZONE_ACTIVE) continue;
            if (index <= zone.barIndex) continue;

            int maxWait = settings.getInteger(MAX_WAIT, 30);
            if ((index - zone.barIndex) > maxWait) continue;

            // Bullish zone retest: price drops into zone
            if (zone.isBullish && enableLong && price <= zone.top && price >= zone.bottom) {
                executeEntry(ctx, instr, settings, true, zone, barTime, price);
                return;
            }

            // Bearish zone retest: price rises into zone
            if (!zone.isBullish && enableShort && price >= zone.bottom && price <= zone.top) {
                executeEntry(ctx, instr, settings, false, zone, barTime, price);
                return;
            }
        }
    }
    ...
    // ==================== ENTRY EXECUTION ====================
    private void executeEntry(OrderContext ctx, Instrument instr, Settings settings,
                              boolean goLong, IFVGZone zone, long barTime, double price)
    {
        var series = ctx.getDataContext().getDataSeries();
        int index = series.size() - 1;
        ...
        if (goLong) {
            ...
            stopPrice = instr.round(zone.bottom - stopBuffer);
            tp1Price = instr.round(entryPrice + tp1Pts);
        } else {
            ctx.sell(qty);
            entryPrice = instr.getLastPrice();
            stopPrice = instr.round(zone.top + stopBuffer);
            tp1Price = instr.round(entryPrice - tp1Pts);
        }

        triggerZoneBottom = zone.bottom;
        triggerZoneTop = zone.top;
        ...
        // Draw entry marker
        var marker = settings.getMarker(goLong ? Inputs.UP_MARKER : Inputs.DOWN_MARKER);
        if (marker != null && marker.isEnabled()) {
            addFigure(new Marker(
                new Coordinate(barTime, goLong ? series.getLow(index) : series.getHigh(index)),
                goLong ? Enums.Position.BOTTOM : Enums.Position.TOP, marker,
                "IFVG " + (goLong ? "L" : "S") + " " + fmt(zone.avg)));
        }

        debug(String.format("=== %s TICK ENTRY === qty=%d, entry=%.2f, stop=%.2f, TP1=%.2f, zone=[%.2f-%.2f]",
            goLong ? "LONG" : "SHORT", qty, entryPrice, stopPrice, tp1Price, zone.bottom, zone.top));
    }

    // ==================== BAR CLOSE (Strategy-level — trade management on bar close) ====================
    @Override
    public void onBarClose(OrderContext ctx)
    {
        ...
        // Track best price (using bar high/low)
        if (isLong) {
        ...
        // Stop loss check (bar close: uses bar low/high, not tick)
